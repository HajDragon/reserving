<div id="selected-day-panel" class="mt-6 rounded-xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-700 dark:bg-zinc-900">
    <div class="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h3 class="text-lg font-semibold text-zinc-900 dark:text-zinc-100">{{ __('Manage Order Items') }}</h3>
            <p class="text-sm text-zinc-600 dark:text-zinc-300">{{ __('Order #:order_id', ['order_id' => $order->id]) }}</p>
        </div>

        <a href="{{ route('reserving.index') }}" class="text-sm font-medium text-zinc-600 underline dark:text-zinc-300">{{ __('Back to Dashboard') }}</a>
    </div>

    @if ($reservations->isEmpty())
        <p class="text-sm text-zinc-600 dark:text-zinc-300">{{ __('No items found in this order.') }}</p>
    @else
        <div class="space-y-4">
            @foreach ($reservations as $reservation)
                ...
                <form method="POST" action="{{ $isRemovalRequest ? route('reservation-removal-requests.update-status', $latestRemovalRequest) : route('reservations.update-status', $reservation) }}" class="ajax-manage-form space-y-3 rounded-lg border p-4 {{ $isRemovalRequest ? 'border-red-200 bg-red-50 dark:border-red-900 dark:bg-red-950/20' : 'border-zinc-200 dark:border-zinc-700' }}">
                    @csrf
                    @method('PATCH')
