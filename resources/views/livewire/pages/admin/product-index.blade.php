<div class="space-y-6 p-6">
        ...
        <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
            ...
            @if ($search !== '')
                <button type="button" wire:click="clearSearch" class="mt-2 text-sm text-zinc-700 underline dark:text-zinc-200">
                    Clear
                </button>
            @endif
        </div>
            ...
            <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                ...
                <tbody class="divide-y divide-zinc-200 dark:divide-zinc-700">
                    @forelse ($products as $product)
                        <tr
                            wire:key="cms-product-row-{{ $product['id'] }}"
                            x-data="{ visible: false }"
                            x-init="requestAnimationFrame(() => { visible = true })"
                            :class="visible ? 'translate-y-0 opacity-100' : 'translate-y-2 opacity-0'"
                            class="transition-all duration-300 ease-out motion-reduce:translate-y-0 motion-reduce:opacity-100"
                        >
                            <td class="px-4 py-3">
                                @if ($product['photo_path'])
                                    <img src="{{ $product['photo_path'] }}" alt="{{ $product['name'] }}" loading="lazy" decoding="async" class="h-10 w-10 rounded-md border border-zinc-200 object-cover dark:border-zinc-700">
                                @else
                            ...
                            <td class="px-4 py-3 text-right text-sm">
                                <div class="flex justify-end gap-3">
                                    <a href="{{ route('cms.products.show', $product['id']) }}" wire:navigate.hover class="text-zinc-600 hover:text-zinc-900 dark:text-zinc-300 dark:hover:text-zinc-100">{{ __('View') }}</a>
                                    <a href="{{ route('cms.products.edit', $product['id']) }}" wire:navigate class="text-zinc-600 hover:text-zinc-900 dark:text-zinc-300 dark:hover:text-zinc-100">{{ __('Edit') }}</a>
                                    <form method="POST" action="{{ route('cms.products.destroy', $product['id']) }}" onsubmit="return confirm('{{ __('Delete this product?') }}')">
                                        @csrf
                                        @method('DELETE')
                ...
                x-init="
                    const observer = new IntersectionObserver((entries) => {
                        if (entries[0] && entries[0].isIntersecting) {
                            $wire.dispatch('cms-products-load-more');
                        }
                    }, { rootMargin: '500px' });
    ...
    <div class="rounded-xl border border-zinc-200 bg-white p-4 text-sm text-zinc-600 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300">
        <div wire:loading wire:target="loadMore">Loading more products...</div>
        @if (! $hasMore)
