...
<head>
    ...
    <title>500 — {{ __("Serverfout") }} — {{ config('app.name') }}</title>
...
<body class="flex min-h-screen items-center justify-center bg-white antialiased dark:bg-zinc-800">
    <div class="mx-auto max-w-md px-6 text-center">
        <div class="mb-6 text-7xl font-bold text-zinc-300 dark:text-zinc-600">500</div>
        <h1 class="mb-4 text-2xl font-semibold text-zinc-900 dark:text-white">{{ __("Er is iets misgegaan") }}</h1>
        <p class="mb-8 text-zinc-500 dark:text-zinc-400">
            {{ __("Er is een fout opgetreden op de server. Probeer het later opnieuw. Als het probleem aanhoudt, neem dan contact op met een beheerder.") }}
        </p>
